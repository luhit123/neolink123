...
const ProgressNoteDisplay: React.FC<ProgressNoteDisplayProps> = ({ note }) => {
    ...
    return (
        <div className="bg-white p-2 sm:p-4 rounded-lg border-2 border-sky-300 shadow-md space-y-3 sm:space-y-4">
            {/* Date and Author */}
            <div className="flex justify-between items-start border-b-2 border-sky-200 pb-2">
                <div>
                ...
                {note.addedBy && (
                    <div className="text-right">
                        <p className="text-xs text-sky-600">Added by</p>
                        <p className="text-sm font-semibold text-slate-800">{note.addedBy}</p>
                    </div>
                )}
            </div>
            ...
            {/* Vital Signs */}
            {hasVitals && (
                <div className="bg-sky-50 border-2 border-sky-200 p-2 sm:p-3 rounded-lg">
                    <h4 className="text-sm font-bold text-sky-700 mb-2 flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-2 text-xs">
                        ...
                        {note.vitals?.crt && (
                            <div className="bg-white p-2 rounded border border-sky-200">
                                <span className="text-sky-600">CRT:</span>
                ...
                <div className="bg-sky-50 border-2 border-sky-200 p-2 sm:p-3 rounded-lg">
                    ...
                    <div className="space-y-2 text-sm">
                        {note.examination?.cns && (
                            <div>
                                <span className="font-semibold text-sky-700">CNS:</span>
                                <p className="text-slate-800 ml-2 mt-1">{note.examination.cns}</p>
                            </div>
                        )}
                        {note.examination?.cvs && (
                            <div>
                                <span className="font-semibold text-sky-700">CVS:</span>
                                <p className="text-slate-800 ml-2 mt-1">{note.examination.cvs}</p>
                            </div>
                        )}
                        {note.examination?.chest && (
                            <div>
                                <span className="font-semibold text-sky-700">CHEST:</span>
                                <p className="text-slate-800 ml-2 mt-1">{note.examination.chest}</p>
                            </div>
                        )}
                        {note.examination?.perAbdomen && (
                            <div>
                                <span className="font-semibold text-sky-700">PER ABDOMEN:</span>
                                <p className="text-slate-800 ml-2 mt-1">{note.examination.perAbdomen}</p>
                            </div>
                        )}
                        {note.examination?.otherFindings && (
                            <div>
                                <span className="font-semibold text-sky-700">OTHER FINDINGS:</span>
                                <p className="text-slate-800 ml-2 mt-1">{note.examination.otherFindings}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="bg-sky-50 border-2 border-sky-200 p-2 sm:p-3 rounded-lg">
                    ...
                    <div className="space-y-2">
                        {note.medications?.map((med, index) => (
                            <div key={index} className="bg-white border border-sky-200 p-2 rounded text-sm">
            ...
            {note.note && (
                <div className="bg-sky-50 border-2 border-sky-200 p-2 sm:p-3 rounded-lg">
                    ...
                    <p className="text-slate-800 text-sm whitespace-pre-wrap">{note.note}</p>
                </div>
            )}
