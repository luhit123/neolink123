/**
 * Run this script to seed admission indications to Firestore
 * Usage: node scripts/runSeedIndications.mjs
 */
...
// Get Firestore instance for the 'neolink' database
...
const ADMISSION_INDICATIONS = [
  ...
  { name: 'Pulmonary Hemorrhage (PICU)', applicableUnits: [Unit.PICU], isActive: true, order: 207 },
  ...
  { name: 'Guillain-Barre Syndrome requiring ventilation', applicableUnits: [Unit.PICU], isActive: true, order: 219 },
  ...
  { name: 'Requiring Mechanical Ventilation (PICU)', applicableUnits: [Unit.PICU], isActive: true, order: 250 },
  ...
  { name: 'Post-extubation Monitoring (HDU)', applicableUnits: [Unit.HDU], isActive: true, order: 301 },
...
async function seedIndications() {
  console.log('🌱 Starting admission indications seeding to Firestore...');
  console.log(`📊 Total indications to seed: ${ADMISSION_INDICATIONS.length}`);

  const indicationsRef = db.collection('admissionIndications');

  // Check if indications already exist
  const existingSnapshot = await indicationsRef.orderBy('order').limit(1).get();

  if (!existingSnapshot.empty) {
    console.log('⚠️ Indications already exist in Firestore. Skipping to avoid duplicates.');
    console.log('💡 To re-seed, first delete the admissionIndications collection from Firebase Console.');
    process.exit(0);
  }

  let successCount = 0;
  let failedCount = 0;
  const userEmail = 'system@neolink.health';
  ...
  for (const indication of ADMISSION_INDICATIONS) {
    try {
      const docRef = indicationsRef.doc();
      batch.set(docRef, {
        ...indication,
        createdAt,
        createdBy: userEmail,
      ...
      if (batchCount >= batchSize) {
        await batch.commit();
        console.log(`✅ Committed batch of ${batchCount} indications (${successCount} total)`);
        batch = db.batch();
        batchCount = 0;
      }
    } catch (error) {
  ...
  if (batchCount > 0) {
    await batch.commit();
    console.log(`✅ Committed final batch of ${batchCount} indications`);
  }
